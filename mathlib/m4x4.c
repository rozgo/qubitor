...
//z = ( far + near ) / p;
//
//matrix[11 = 2 / w; matrix[12 = 0;     matrix[13 = 0;      matrix[14 = -x;
//matrix[21 = 0;     matrix[22 = 2 / h; matrix[23 = 0;      matrix[24 = -y;
//matrix[31 = 0;     matrix[32 = 0;     matrix[33 = -2 / p; matrix[34 = -z;
//matrix[41 = 0;     matrix[42 = 0;     matrix[43 = 0;      matrix[44 = 1;
...
void m4x4_look_at ( m4x4_t matrix, const vec3_t eye, const vec3_t at, const vec3_t up )
{
    ...
    VectorNormalize ( f, f );
    VectorNormalize ( up, nup );
    CrossProduct ( f, nup, s );
    CrossProduct ( s, f, u );
    ...
    vec3_t tv = { -eye[0], -eye[1], -eye[2] };
    m4x4_translation_for_vec3 ( t, tv );
    m4x4_multiply_by_m4x4 ( matrix, t );
}
...
int m4x4_un_project ( const vec3_t screen_pos, const m4x4_t view, const m4x4_t proj, const int viewport[4], vec3_t pos )
{
    ...
    m4x4_multiply_by_m4x4 ( final, proj );
    m4x4_multiply_by_m4x4 ( final, view );
    if ( m4x4_invert ( final ) ) return 0;
    ...
    out[0] = (out[0] - viewport[0]) / viewport[2];
    out[1] = (out[1] - viewport[1]) / viewport[3];
