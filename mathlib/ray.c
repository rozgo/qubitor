...
/* revise
int dist_point_line ( vec3_t point, vec3_t line_start, vec3_t line_end, vec_t* dist )
{
    float line_mag;
    float u;
    vec3_t line_sub;
    ...
    VectorSubtract ( line_end, line_start, line_sub );
    
    line_mag = VectorLength ( line_sub );
    ...
    VectorSubtract ( point, intersection, point_int );
    *dist = VectorLength ( point_int );
